...
import (
	...
	"github.com/DownerCase/ecal-go/cmd/monitor/internal/protobuf/order"
...
func (o Options) rangeAnyMessage(p *protopath.Values, m protoreflect.Message, push, pop func(protopath.Values) error) (ok bool, err error) {
	...
	fds := md.Fields()
	url := m.Get(fds.ByNumber(Any_TypeUrl_field_number)).String()
	val := m.Get(fds.ByNumber(Any_Value_field_number)).Bytes()
	...
	if !bytes.Equal(b1, b2) {
		m.Set(fds.ByNumber(Any_Value_field_number), protoreflect.ValueOfBytes(b2))
	}
